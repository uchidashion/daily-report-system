...
public class EmployeeService extends ServiceBase{
    ...
    public EmployeeView findOne(String code, String plainPass, String pepper) {
        ...
        /**
         * 画面から入力された従業員の更新内容を元にデータ一件作成し、従業員テーブルを更新する
         * @param ev 画面から入力された従業員の登録内容
         * @param pepper pepper文字列
        ...
        public List<String> update(EmployeeView ev, String pepper){
            ...
            if(!savedEmp.getCode().equals(ev.getId())) {
                //社員番号を更新する場合
        ...
        public Boolean validateLogin(String code, String plainPass, String pepper) {
            ...
            //社員暗号とパスワードの値がある場合はデータが取得できるか認証を行う
        ...
        private void create(EmployeeView ev) {
            ...
            em.getTransaction().begin();  //DBに登録を始める
            em.persist(EmployeeConverter.toModel(ev));  //DTOモデルのインスタンスデータを一件登録する
            em.getTransaction().commit();  //登録を確定する
        }
        ...
        private void update(EmployeeView ev) {
            ...
            em.getTransaction().begin();  //DBの登録を始める
            Employee e = findOneInternal(ev.getId());  //idを条件にデータを一件取得し、Employeeのインスタンスで返却する
            EmployeeConverter.copyViewToModel(e, ev);  //Viewモデルの全フィールドの内容をDTOモデルのフィールドにコピーする
            em.getTransaction().commit();  //DBの登録を確定する
