...
public class ReportService extends ServiceBase {
    ...
    /**
     * 指定した従業員が作成した日報データを、指定したページ数の一覧画面に表示する分取得しReportViewのリストで返却する
     * @param employee 従業員
     * @param page ページ数
     ...
     * @return 一覧画面に表示するデータのリスト
     */
    public List<ReportView> getAllPerPsage(int page){
    ...
    /**
     * 画面から入力された日報の登録内容を元にデータを一件作成し、日報データに登録をする
     * @param rv 日報の登録内容
     * @return バリデーションで発生したエラーのリスト
     */
    public List<String> create(ReportView rv){
        List<String> errors = ReportValidator.validate(rv);
        //もし入力値がある場合は日報テーブルに登録する
        if(errors.size() == 0) {
    ...
    public List<String> update(ReportView rv){
        ...
        //バリデーションで発生したエラーを返却（エラーがなければ０件のリスト）
        return errors;
    }
