...
public class FrontController extends HttpServlet {
     ...
     * （例：パラメータがaciton=Employeeの場合、action.EMployeeActionオブジェクト）
     * @param request リクエスト
     * @param response レスポンス
     * @return
     */
    @SuppressWarnings({ "unchecked", "rawtypes" }) //コンパイラ警告を抑制
    private ActionBase getAction(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            //該当するActionBaseのオブジェクトを作成（例：リクエストからパラメータaction=Employeeの場合、actions.EmployeeActionオブジェクト）
